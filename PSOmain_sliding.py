...
import time
from geneticAlgorithms import pyswarmsPSO_sliding
...
def build_container_from_solution(solution, rectangles):
    n = len(rectangles)
...
if __name__ == "__main__":
    ...
    pso = pyswarmsPSO_sliding.PySwarmsPSO(rectangles, width, config)
    ...
    def on_gen_changed(gen_idx):
        #solution = result
        container = build_container_from_solution(solution, rectangles)
        if not container.rectangles:
